...
proc saveFile(ctx: Context, fileFieldName: string, mediaDirectory: string, subdir: Option[
    string]): string =
...
proc assertFormDataCompleteness[T: Model](
  ctx: Context,
  model: typedesc[T],
  skipIdField: static bool = false
) =
    ...
    if formValueStr.isNone():
      ...
      # Model Field is any other Field and must not be nil (is not Optional) and Form value is not nil --> Parse form value into field
        ...
        when isFileField:
          ...
          const fileSubdir: Option[string] = when hasSubDirectory:
              some(getCustomPragmaVal(dummyValue, subdir))
            else:
              none(string)
